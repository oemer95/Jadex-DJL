...
class Vehicle():
    ...
    def __init__(self):
        ...
        self.energy = 20
        self.n_ctrips = 0
        self.state = np.array([self.energy, self.n_ctrips, 0, 0])
    ...
    def create_trips(self):
        ...
        subset = subset.round(0)  # in the DQN high version we round to 1
    ...
    def reset(self):
        ...
        self.n_ctrips = 0
        self.energy = 20
        self.state = np.array([20, 0, self.trips[0][0], self.trips[0][1]])
    ...
    def commit(self):
        ...
        done = False # if it turns true the episode ends
        ...
        if self.trips:
            self.trips.pop(0)
            if self.trips:
                self.state = np.array([self.energy, self.n_ctrips, self.trips[0][0], self.trips[0][1]])
            else:
                done = True
